...
if __name__ == '__main__':
    ...
    TheSystem.LoadFile(os.getcwd() + "\\Sloan FVC-3o9a-Final_cs.zmx", False)
    ...
    def runOne(Xdata, Ydata, outputDataFile):
        ...
        for k in range(len(Xdata)):
            ...
            for p in range(len(Xdata[k])):
                ...
                TheMCE.GetOperandAt(7*p+4).GetOperandCell(k+1).DoubleValue = 0.985
                TheMCE.GetOperandAt(7*p+5).GetOperandCell(k+1).DoubleValue = 0.985
    ...
    #for seed in [3,4]:
        ...
        if seed == 0:
            ...
            fileName = "apoFVC_colby.csv"
            Xdata = dataGen.Xdata_sloan
            Ydata = dataGen.Ydata_sloan
        elif seed == 1:
            # do full hex
            fileName = "apoFVC_allHex.csv"
            Xdata = dataGen.Xdata_allHex
            Ydata = dataGen.Ydata_allHex
        #elif seed == 2:
            #fileName = "apoFVC_fidHex.csv"
            #Xdata = dataGen.Xdata_fidHex
            #Ydata = dataGen.Ydata_fidHex
        else:
            fileName = "apoFVC_uniform_seed_%i.csv"%seed
    ...
    #Save file
    TheSystem.SaveAs(os.getcwd() + '\\sdssjunk.zmx')
